...
import { Car, Search, ChevronDown, Menu, Sun, Moon, Facebook, Instagram, Twitter } from 'lucide-react'
            ...
            <main className="container mx-auto px-4 py-20 md:py-24">
                ...
                <div className="max-w-4xl mx-auto mb-16">
                    <div className={`flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 ${darkMode ? 'bg-gray-800' : 'bg-white'} p-6 rounded-xl shadow-lg`}>
                        <Input type="text" placeholder="Search by make, model, or keyword" className={`flex-grow ${darkMode ? 'bg-gray-700 text-white' : 'bg-gray-100 text-gray-900'}`} />
                        <Select>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
                    {[1, 2, 3, 4, 5, 6].map((i) => (
                        <div key={i} className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl overflow-hidden shadow-lg`}>
                            <Image src={`https://hebbkx1anhila5yf.public.blob.vercel-storage.com/car-${(i % 3) + 1}-Ld5Hy7Ue0Ue9Ue9Ue9Ue9Ue9Ue9Ue9.jpg`} alt={`Car ${i}`} width={400} height={250} className="w-full h-48 object-cover" />
                            <div className="p-6">
                                <h3 className={`text-xl font-bold mb-2 ${darkMode ? 'text-gray-100' : 'text-gray-800'}`}>2023 Model Y</h3>
                                <p className={`${darkMode ? 'text-gray-300' : 'text-gray-600'} mb-4`}>Electric SUV with advanced features and long range.</p>
                                <div className="flex justify-between items-center">
                                    <span className={`text-2xl font-bold ${darkMode ? 'text-blue-400' : 'text-blue-600'}`}>$55,000</span>
